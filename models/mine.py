...
class Mine(nn.Module):
    ...
    def forward(self, x, z, z_marg=None):
        ...
        return -t + second_term +0.1*second_term**2
...
class T(nn.Module):
    def __init__(self, x_dim, z_dim):
        super().__init__()
        self.layers = CustomSequential(ConcatLayer(), nn.Linear(x_dim + z_dim, 100),
                                       nn.ReLU(),
                                       nn.Linear(100, 100),
                                       nn.ReLU(),
                                       nn.Linear(100, 1))
...
class MutualInformationEstimator(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        mi = -loss
        if batch_idx == 50:
            self.raw_data.append(float(mi))
        self.raw_data2.append((batch_idx,float(mi)))
    ...
    def raw_data_getter(self):
        raw_data = self.raw_data
        return raw_data
        
         
    def raw_data_getter2(self):
        raw_data = self.raw_data2
        return raw_data2
